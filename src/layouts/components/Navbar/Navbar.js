...
const Navbar = () => {
  ...
  return (
    <nav className="relative flex justify-between gap-8 rounded-xl px-8 dark:bg-dark-regular">
      {isExist && (
        <div
          style={{ left: positionActive + "px" }}
          className={`absolute bottom-0 mx-8 h-[56px] w-[56px] translate-y-1/2 rounded-full border-[6px] border-dark-bold bg-primary transition-all duration-300`}
        >
          <div className="absolute top-1/2 -left-[2px] h-4 w-4 -translate-y-full -translate-x-full rounded-br-full bg-transparent shadow-[0_10px_0_0_rgb(21,26,35)]"></div>

          <div className="absolute top-1/2 -right-[2px] h-4 w-4 -translate-y-full translate-x-full rounded-bl-full bg-transparent shadow-[0_10px_0_0_rgb(21,26,35)]"></div>
        </div>
      )}
